            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    ...
                    <div class="card-form-center">
                        <!-- form start -->
                        <form role="form" action="{{ route('update-city',$city->id) }}" method="POST" enctype="multipart/form-data">
                            {{@csrf_field()}}
                            {{method_field('PUT') }}
                            <div class="card-body">
                                        ...
                                        <div class="col-9">
                                            <select class="form-control" name="state_id">
                                                @foreach($allStates as $state)
                                                    <option value="{{$state->id}}" @if($state->id==$city->state->id) selected="" @endif>{{$state->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group">
                                    <div class="col-12 form-inner-block">
                                        ...
                                        <div class="col-9">
                                            <input type="checkbox" name="status" id="status" class="" value="{{$city->status}}" {{ $city->status==1 ? 'checked' : ''}}>
                                        </div>
                                    </div>
                                </div>
